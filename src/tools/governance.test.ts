...
describe('Governance Tools', () => {
    ...
    it('Free tier precheck exceeding limit', async () => {
        const toolHandler = mockServer.tool.mock.calls.find(c => c[0] === 'ua_precheck')[3];
    ...
    it('Precheck escalating to elicitation', async () => {
        ...
        mockServer.server.elicitInput.mockResolvedValue({
            answers: { confirm: 'i understand and proceed', reason: 'urgent fix' }
        });
    ...
    it('Precheck on critical-path target with <=10 impacted returns MEDIUM risk and does NOT elicit', async () => {
        const toolHandler = mockServer.tool.mock.calls.find(c => c[0] === 'ua_precheck')[3];
        (validateLicense as any).mockResolvedValue({ tier: 'Pro' });
        ...
        expect(result.isError).toBeUndefined();
        expect(result.content[0].text).toContain('[APPROVED] Risk level: MEDIUM');
        expect(result.content[0].text).toContain('Change touches a critical-path file (auth)');
