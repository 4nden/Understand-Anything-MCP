...
export function registerGovernanceTools(server: McpServer) {
    server.tool(
        "ua_precheck",
        "Pre-flight architectural risk check. MUST be called before modifying any critical systems or refactoring.",
        {
        ...
        async ({ target }) => {
            ...
            // Elicitation for HIGH risk
            if (riskLevel === 'HIGH') {
                try {
                    const confirm = await server.server.elicitInput({
                        mode: "form",
                        message: `WARNING: Modifying ${target} poses a HIGH architectural risk.\nRisk Factors:\n${riskFactors.map(f => '- ' + f).join('\n')}`,
                        requestedSchema: {
                            type: "object",
                            properties: {
                                reason: {
                    ...
                    const confirmText = confirm.answers?.confirm || '';
                    if (String(confirmText).toLowerCase().trim() === 'i understand and proceed') {
                        return { content: [{ type: "text", text: `[APPROVED] User authorized the high-risk change. Justification: ${confirm.answers?.reason}\nImpacted Files: ${impactedFiles.join(', ')}` }] };
                    } else {
                        return { content: [{ type: "text", text: `[ABORTED] User did not authorize the high-risk change.` }] };
                    }
                } catch (e: any) {
                    return { content: [{ type: "text", text: `[BLOCKED] High risk detected, but elicitation failed or was cancelled: ${e.message}` }], isError: true };
                }
            }
    ...
    const handleRules = async () => {
        ...
        if (rules.length === 0) {
            return {
                content: [{ type: "text", text: `No rules found in .ua-rules.json. To get started, create a .ua-rules.json file in the root of your project.` }]
            };
        }
